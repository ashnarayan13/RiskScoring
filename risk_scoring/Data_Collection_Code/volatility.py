...
def volatility(inp):
    print(inp)
    workbook_volatility = xlrd.open_workbook("/home/ashwath/PycharmProjects/risk_scoring/src/DataSet_Final.xlsx")
    sheet = workbook_volatility.sheet_by_name("TS")
    allinfo = []
    const = inp * 2
    tot = 0
    ctr = 0
...
company = "/home/ashwath/FinancialModel/final_sets/DataSet_Final.xlsx"
workbook = xlrd.open_workbook(company)
comp_sheet = workbook.sheet_by_name("CDAX")
companies = []
for lim in range(2, comp_sheet.nrows):
    print(str(lim - 1) + " " + str(comp_sheet.cell_value(lim, 3)))
    companies.append(str(comp_sheet.cell_value(lim, 3)))
...
for lim in range(2, comp_sheet.nrows):
    print(comp_sheet.cell_value(lim, 3))
    wb_sam_sheet1.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet2.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet3.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet4.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet5.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet6.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet7.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet8.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
    wb_sam_sheet9.write(lim, 0, str(comp_sheet.cell_value(lim, 3)))
